...
class ChartGenerator:
    ...
    def suggest_chart_type(self, query: str, tools_used: List[str]) -> str:
        ...
        if any(tool in tools_used for tool in ["get_daily_revenue", "get_revenue_by_date_range"]):
        ...
        if "get_customer_segments_breakdown" in tools_used:
            return "pie"
        
        # Top items/customers - horizontal bar chart
        if any(tool in tools_used for tool in ["get_top_customers_by_spending", "get_top_menu_items_by_orders", "get_top_menu_items_by_revenue"]):
            return "horizontal_bar"
        
        # Status/breakdown data - pie or bar
        if any(tool in tools_used for tool in ["get_order_status_breakdown", "get_order_types_breakdown", "get_payment_methods_breakdown"]):
